/* =============================================================
   ...
   Modules (IIFE compartments):
     ...
     Sound    â€“ local click / win sounds
     FX       â€“ confetti + fireworks on canvas
     Sequence â€“ goal-order generators
     Game     â€“ core puzzle model
     UI       â€“ screen routing, rendering
   ============================================================= */
(() => {
'use strict';
...
/* -------------------- SEQUENCE GENERATORS --------------------
   ...
   order[1], â€¦, and the blank goes at order[N*N - 1].
...
const Game = (() => {
  ...
  /* Click tile at index â€” slide entire row/col toward blank.
  ...
  function undo() {
    ...
    if (moves) {
      ...
      state.moves -= 2; // slideToward incremented; undo shouldn't count â†’ subtract both
    }
    return moves;
  }
  function redo() {
...
const UI = (() => {
  ...
  function renderDashboard() {
    ...
    ).join('') || '<p style="color:var(--muted)">No records yet â€” play a game!</p>';

    const historyRows = s.history.slice(0, 8).map(h =>
      `<div class="hs-row"><span class="lbl">${h.size}Ã—${h.size} ${h.mode}</span><span class="val">${h.moves}m / ${formatTime(h.time)}</span></div>`
  ...
  function attachBoardSwipe() {
    ...
    boardEl.addEventListener('pointerup', e => {
      ...
      // slide *the blank toward us* means click on tile in that line â€” easier: just use sIdx
  ...
  function init() {
    ...
    $('#btn-share').addEventListener('click', async () => {
      const st = Game.get();
      const text = `I solved a ${st.size}Ã—${st.size} ${st.mode} puzzle in ${st.moves} moves & ${formatTime(st.elapsed)} on Number Puzzle Pro!`;
      try {
