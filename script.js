/* =============================================================
   Number Puzzle Pro — Premium Core Engine
   Pure vanilla JS sliding-puzzle engine.
   Modules (IIFE compartments):
     Storage  – localStorage persistence & validation
     Sound    – tactile click / win / error acoustics
     FX       – high-performance canvas confetti + fireworks
     Sequence – goal-order multi-sequence layout generators
     Game     – relational sliding model mechanics
     UI       – routing, presentation paint cycles & interactions
   ============================================================= */
(() => {
  'use strict';
  ...
  const Storage = (() => {
    ...
    function load() {
      try {
        ...
        console.warn('Corrupted save file detected, resetting profile parameters.', e);
        return structuredClone(DEFAULT);
      }
    }
    function save() {
      try { localStorage.setItem(STORAGE_KEY, JSON.stringify(data)); }
      catch (e) { console.warn('Persistent save transactional failure', e); }
    }
    ...
    return {
      ...
      importJSON(json) {
        ...
        if (!parsed || typeof parsed !== 'object') throw new Error('Invalid schema format');
  ...
  const Sound = (() => {
    ...
    function playFile(audioObject) {
      ...
      audioObject.play().catch(error => {
        // Silent catch for initial browser audio context lifecycle restrictions
        console.debug("Acoustic stream interrupted by system security layer", error);
      });
    }
  ...
  function vibrate(ms = 12) {
    if (Storage.get().settings.vibration && navigator.vibrate) {
      try { navigator.vibrate(ms); } catch {}
    }
  }

  /* -------------------- SEQUENCE MATH GENERATORS -------------------- */
  const SequenceGen = {
    Classic(n) {
  ...
  /* -------------------- HIGH-PERFORMANCE FX ENGINE (FAIL-SAFE) -------------------- */
  const FX = (() => {
    let canvas, ctx, raf, parts = [], running = false;
    
    function init(c) {
      if (!c) {
        console.warn("FX Particle engine context missing. Continuing initialization loop structural lifecycle safely.");
        return;
      }
  ...
  /* -------------------- USER INTERFACE MODULE & PRESENTATION PAINT CORES -------------------- */
  const UI = (() => {
    ...
    function renderPreview() {
      ...
      for (let i = 0; i < n * n; i++) {
        ...
        if (labels[i] === 0) { t.classList.add('blank'); }
          ...
          t.style.backgroundPosition = `${(c/Math.max(1, n-1))*100}% ${(r/Math.max(1, n-1))*100}%`;
        } else {
    ...
    function renderDashboard() {
      ...
      ).join('') || '<p style="color:var(--muted)">No games recorded</p>';
    ...
    function loadPhotoFile(file) {
      ...
      if (!/^image\/(png|jpe?g|webp)$/.test(file.type)) {
        toast('Unsupported image format'); return;
      }
    ...
    function attachBoardSwipe() {
      ...
      boardEl.addEventListener('pointerup', e => {
        ...
        if (Math.hypot(dx, dy) < 15) { sIdx = -1; return; } 
    ...
    function startGame() {
      const s = Storage.get().settings;
      if (s.mode === 'Photo' && !photoDataURL) { toast('Please upload a photo first'); return; }
      Game.newGame({
    ...
    function init() {
      ...
      $('#btn-back-home').addEventListener('click', async () => {
        if (Game.get() && !Game.get().finished && Game.get().moves > 0) {
          const ok = await confirm({ title: 'Leave game?', message: 'Progress will be safely preserved.', okText: 'Leave' });
          if (!ok) return;
        }
      ...
      $('#btn-hint').addEventListener('click', () => {
        ...
        toast('Tactical suggestion applied');
      });
      ...
      $('#btn-shuffle').addEventListener('click', async () => {
        const ok = await confirm({ title: 'Reshuffle Board?', message: 'Current progression tracking parameters will be cleared.', okText: 'Shuffle' });
        if (ok) startGame();
      });
      
      $('#btn-restart').addEventListener('click', async () => {
        const ok = await confirm({ title: 'Restart Vector?', message: 'Reset current moves and elapsed sequence timers.', okText: 'Restart' });
        if (ok) startGame();
      });
      ...
      $('#btn-share').addEventListener('click', async () => {
        const st = Game.get();
        const text = `I solved a ${st.size}×${st.size} ${st.mode} mode configuration in ${st.moves} moves within ${formatTime(st.elapsed)} on Number Puzzle Pro!`;
        try {
          if (navigator.share) await navigator.share({ title: 'Number Puzzle Pro', text });
          else { await navigator.clipboard.writeText(text); toast('Copied profile matrix metrics'); }
        } catch {}
      });
    ...
    $('#btn-export-data').addEventListener('click', () => {
      ...
      a.download = 'number-puzzle-pro-backup.json'; 
      a.click();
    });
    ...
    $('#btn-reset-data').addEventListener('click', async () => {
      const ok = await confirm({ 
        ...
        message: 'All scores, settings and achievements will be erased.', 
        okText: 'Reset' 
      });
      if (ok) { 
        Storage.reset(); 
